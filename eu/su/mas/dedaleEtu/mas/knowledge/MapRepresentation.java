...
public class MapRepresentation implements Serializable {
	...
	public List<MySerializableNode> getNodes() {
		return this.g.nodes().map(n -> new MySerializableNode(n.getId(), (String)n.getAttribute("ui.class"))).collect(Collectors.toList()); //map(Node::getId).collect(Collectors.toList());
	}
	
	public List<MySerializableEdge> getEdges() {
		return this.g.edges().map(edge -> new MySerializableEdge(edge.getSourceNode().getId(), edge.getTargetNode().getId()))
		.collect(Collectors.toList());
	}
	
	public List<MySerializableNode> getDifferenceNodes(List<MySerializableNode> known) {
		return this.g.nodes().map(node -> new MySerializableNode(node.getId(), (String)node.getAttribute("ui.class")))
				.filter(node -> !known.contains(node)).collect(Collectors.toList());
	}


	public List<MySerializableEdge> getDifferenceEdges(List<MySerializableEdge> known) {
		return this.g.edges().map(edge -> new MySerializableEdge(edge.getSourceNode().getId(), edge.getTargetNode().getId()))
				.filter(edge -> !known.contains(edge)).collect(Collectors.toList());
	}
